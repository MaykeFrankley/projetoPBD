...
BEGIN
    ...
    IF unidade = 'Bimestral' AND valorUnidade < 5 THEN
		SELECT AVG(media) INTO mediaVar FROM argus.notas WHERE notas.codAluno = codAluno AND notas.codDisciplina = codDisciplina AND
						notas.serie = serie AND notas.anoLetivo = anoLetivo AND notas.valorUnidade < 5 AND notas.situacao = "Atualizado";
	...
	IF unidade = 'Trimestral' AND valorUnidade < 4 THEN
		SELECT AVG(media) INTO mediaVar FROM argus.notas WHERE notas.codAluno = codAluno AND notas.codDisciplina = codDisciplina AND
						notas.serie = serie AND notas.anoLetivo = anoLetivo AND valorUnidade < 4 AND notas.situacao = "Atualizado";
		UPDATE MediaGeralAluno SET MediaGeralAluno.media = mediaVar, MediaGeralAluno.situacao = "Pendente" 
				WHERE MediaGeralAluno.codAluno = codAluno AND MediaGeralAluno.codDisciplina = codDisciplina 
                AND MediaGeralAluno.serie = serie AND MediaGeralAluno.anoLetivo = anoLetivo;
		IF mediaVar >= 7 THEN
...
AS SELECT DISTINCT pessoas.codPessoa, pessoas.Nome, pessoas.Naturalidade, pessoas.Dt_nascimento, pessoas.CPF, Alunos.nomeMae, Alunos.nomePai, Alunos.codResponsavelFin, Pessoas.Status, `Aluno-Turma`.situacao
FROM Pessoas
INNER JOIN Alunos
...
AS SELECT `sessaopedagogica`.`codPedagogo` AS `codPedagogo`,
        ...
        `sessaopedagogica`.`data` AS `Data de atendimento`,
        `sessaopedagogica`.`status` AS `status`
FROM Pessoas
INNER JOIN Alunos
ON pessoas.codPessoa = Alunos.codPessoa
INNER JOIN SessaoPedagogica
ON Alunos.codPessoa = SessaoPedagogica.codAluno;
