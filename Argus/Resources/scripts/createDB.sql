...
CREATE TABLE IF NOT EXISTS `argus`.`Notas` (
  ...
  INDEX `fk_tipo_notas_idx` (`unidade` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_codDisciplina_notas`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_tipo_notas`
    FOREIGN KEY (`unidade`)
    REFERENCES `argus`.`Curriculo` (`Tipo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
	...
	LOOP
		...
		ELSE
			IF (SELECT tipo FROM curriculo WHERE codCurriculo = curriculo) = 'Bimestral' THEN
				INSERT INTO Notas(codAluno, codDisciplina, serie, anoLetivo, unidade, valorUnidade, media, foiFinal, situacao) 
				VALUES(alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 1, 0.0, 0, "Pendente"),
                (alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 2, 0.0, 0, "Pendente"),
                (alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 3, 0.0, 0, "Pendente"),
                (alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 4, 0.0, 0, "Pendente"),
                (alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 5, 0.0, 0, "Pendente");
            ELSE
				INSERT INTO Notas(codAluno, codDisciplina, serie, anoLetivo, unidade, valorUnidade, media, foiFinal, situacao) 
				VALUES(alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 1, 0.0, 0, "Pendente"),
                (alunoCodigo, codDisciplinaNota, ano, anoLetivoCur,(SELECT tipo FROM curriculo WHERE codCurriculo = curriculo), 2, 0.0, 0, "Pendente"),
