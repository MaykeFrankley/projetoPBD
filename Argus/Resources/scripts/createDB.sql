...
CREATE TABLE IF NOT EXISTS `argus`.`Alunos` (
  ...
  `Status` ENUM("Ativo", "Inativo") NOT NULL,
  PRIMARY KEY (`codPessoa`, `codMae`, `codPai`, `codReponsávelFin`),
...
CREATE TABLE IF NOT EXISTS `argus`.`Curriculo-Disciplina` (
  ...
  `ano` INT NOT NULL,
  `anoLetivo` INT NOT NULL,
  PRIMARY KEY (`codCurriculo`, `codDisciplina`, `cargaHoraria`, `ano`, `anoLetivo`),
...
CREATE TABLE IF NOT EXISTS `argus`.`Professores-Disciplinas` (
  ...
  PRIMARY KEY (`codProfessor`, `codDisciplina`, `codCurriculo`, `cargaHoraria`, `ano`, `anoLetivo`),
  INDEX `index_curriculo_PD` (`codCurriculo` ASC) VISIBLE,
  INDEX `index_cargaHoraria_PD` (`cargaHoraria` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_codCurriculo_inner`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_cargaHoraria_inner`
    FOREIGN KEY (`cargaHoraria`)
    REFERENCES `argus`.`Curriculo-Disciplina` (`cargaHoraria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
